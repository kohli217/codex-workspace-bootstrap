...
When multiple AI instruction files exist, `cwb` reads executable-looking commands and checks them against repository evidence. It understands nested `AGENTS.md` / `AGENTS.override.md`, nested `CLAUDE.md`, nested `GEMINI.md`, nested Cursor `.cursor/rules`, and common path-specific frontmatter such as Copilot `applyTo` and rule `globs`. It can flag:
...
```powershell
py -m pip install "https://github.com/kohli217/codex-workspace-bootstrap/releases/download/v0.5.2/codex_workspace_bootstrap-0.5.2-py3-none-any.whl"
```
