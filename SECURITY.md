...
The Cloudflare workflow-dispatch credential must be a fine-grained personal access token restricted to `kohli217/codex-workspace-bootstrap` with Actions write and Variables write. It is uploaded once as the Worker secret `CWB_DISPATCH_TOKEN` and is never read back by the deployment script. The deployed Worker uses Variables write only to pin the trusted `CWB_TOKEN_ENDPOINT` during setup and uses Actions write for workflow dispatch. The token must never be committed or logged.
